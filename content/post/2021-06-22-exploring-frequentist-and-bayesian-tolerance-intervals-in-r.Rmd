...
Now let's look at the Bayesian approach.  Note that this is completely differnt example from above, not a continuation.  The desired coverage for this case is a little bit different - 95%, and we'll aim for a 90% confidence level (for the frequentist version that comes later) and we'll try to capture the 90% most credible estimates for the Bayesian version.
