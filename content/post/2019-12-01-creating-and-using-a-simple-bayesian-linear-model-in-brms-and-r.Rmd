...
If we further parameterize \mu, we can do some neat things like move the mean around with the predictor variable.  This is a pretty key concept - *you move the mean of the outcome variable around by parameterizing it.  If we make it a line then it will move linearly with the predictor variable.*  The real data will still have a spread once the \sigma terms is folded back in, but we can think of the whole distribution shifting up and down based on the properties of the line.
